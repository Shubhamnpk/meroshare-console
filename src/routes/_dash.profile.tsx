...
export const Route = createFileRoute("/_dash/profile")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Your MeroShare demat account details, contact information and renewal dates.",
      },
...
function ProfilePage() {
  ...
  const identity: [string, string, string][] = [
    ["Name", name, ""],
    ...(s(d.profileName) !== name
      ? [["Profile name", s(d.profileName), ""] as [string, string, string]]
      : []),
    ["Gender", s(d.gender), ""],
    ["PAN", s(d.panNumber), ""],
    ...
    [
      "Demat / BOID",
      demat === "—" ? s(d.boid) : `${demat}${d.boid && d.boid !== demat ? ` · ${d.boid}` : ""}`,
      "demat",
    ],
  ];
  ...
  const account: [string, string, string][] = [
    ["Account number — Capital", `${s(d.accountNumber)} — ${s(d.capital)}`, ""],
  ...
  const avatarUrl =
    typeof d.imagePath === "string" && d.imagePath.trim() !== "" ? d.imagePath : null;
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Details registered with your depository participant.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-4 rounded-2xl border border-border/70 bg-card p-5">
        ...
        <div className="flex flex-wrap gap-2">
          <InfoBadge
            label="Dashboard"
            value={d.renderDashboard ? "Enabled" : "Disabled"}
            tone={d.renderDashboard ? "ok" : "muted"}
          />
