...
function exportSummaryCsv(scrip: string, rows: PurchaseSourceItem[]) {
  ...
  const header = ["SN", "Scrip", "Post date", "Quantity", "Source", "User price", "User cost", "Remarks", "Updated"].map(esc).join(",");
...
function SummaryTable({ scrip, rows }: { scrip: string; rows: PurchaseSourceItem[] }) {
  if (rows.length === 0) {
    return <EmptyBlock title="No summary entries" description={`No calculated WACC entries for ${scrip}.`} />;
  }
  const totalUnits = rows.reduce((s, r) => s + Math.max(0, toNumber(r.quantity ?? r.transactionQuantity)), 0);
          ...
          <tbody className="divide-y divide-border/60">
            {rows.map((r, idx) => (
              <tr key={`${r.postDate}-${idx}`} className="hover:bg-accent/30">
                ...
                <td className="num px-4 py-2.5 text-right">{formatQty(toNumber(r.quantity ?? r.transactionQuantity))}</td>
                ...
                <td className="num px-4 py-2.5 text-right font-medium">{formatNpr(toNumber(r.userCost))}</td>
                <td className="hidden px-4 py-2.5 text-muted-foreground sm:table-cell">{r.remarks ?? "—"}</td>
                <td className="hidden px-4 py-2.5 text-muted-foreground md:table-cell">{formatDate(r.updatedDate)}</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">Pending purchase source entries and your completed WACC calculation.</p>
      </div>
      ...
      {pending.isPending ? (
          ...
          <div className="space-y-4">
            ...
            <ul className="divide-y divide-border/60 overflow-hidden rounded-2xl border border-border/70 bg-card">
              {(pending.data.waccUpdateResponse ?? []).map((row, idx) => (
                <li key={`${row.transactionDate}-${idx}`} className="flex items-center justify-between gap-4 px-4 py-3 text-sm">
                  <div>
                    <p className="font-semibold">{row.scrip ?? scrip}</p>
                    <p className="text-xs text-muted-foreground">{formatDate(row.transactionDate)} · {row.purchaseSource ?? row.source ?? "—"}</p>
                  </div>
                  <div className="text-right">
                    <p className="num font-medium">{formatQty(toNumber(row.transactionQuantity ?? row.quantity))} units</p>
                    <p className="num text-xs text-muted-foreground">{formatNpr(toNumber(row.rate))} / unit</p>
                  </div>
                </li>
              ))}
            </ul>
        ...
        <EmptyBlock title="Select a scrip" description="Pick a scrip above to load its purchase source entries." />
      )}
    </div>
  );
}
